...
use btc_validation::is_beyond_last_checkpoint;
...
const MAX_BLOCKS_BYTES: usize = 2_000_000 - MAX_NEXT_BYTES;
...
pub struct GetSuccessorsRequest {
    /// Hash of the most recent stable block in the Bitcoin smart contract.
    pub anchor: BlockHash,
    /// Most recent block hashes that have been processed by the smart contract.
...
pub struct GetSuccessorsResponse {
    ...
    /// Next set of headers to be sent to the smart contract.
...
mod test {
    ...
    async fn test_get_successors_multiple_blocks_out_of_order() {
        ...
        // Block 1 is not in the cache yet. The Bitcoin virtual smart contract requires that the blocks
